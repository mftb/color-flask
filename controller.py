from repository import ColorRepository, ColorNotFoundException, ColorIOException, ColorAlreadyExistsException
from controller import Controller
from validator import color_validator


class ColorController:

    def __init__(self):
        self.__repository = ColorRepository()

    def all_colors(self):
        return self.__repository.all_colors(), 200

    def get_color(self, color_name):
        try:
            return self.__repository.get_color(color_name), 200
        except ColorNotFoundException:
            return {"error": "Color not found"}, 404

    def add_color(self, color):
        try:
            color_validator(color)
            self.__repository.insert_color(color)
            return {"message": "Color added"}, 201
        except ColorIOException:
            return {"message": "Error while saving color"}, 503
        except ColorAlreadyExistsException:
            return {"message": "Color already exists"}, 409
        except TypeError:
            return {"message": "Invalid color data"}, 400
        except KeyError:
            return {"message": "Invalid color data"}, 400
